...
if (nproc>1) then
	if (my_rank==0 ) then
		value_send=8
		call MPI_ISEND(value_send,1,MPI_INT,my_rank+1,tag,MPI_COMM_WORLD,request_send,ierror)
		call MPI_WAIT(request_send,status,ierror)
		call MPI_IRECV(value_receive,1,MPI_INT,nproc-1,MPI_ANY_TAG,MPI_COMM_WORLD,request_recv,ierror)
	...
	elseif (my_rank>0 .and. my_rank<nproc-1) then
		call MPI_IRECV(value_receive,1,MPI_INT,my_rank-1,MPI_ANY_TAG,MPI_COMM_WORLD,request_recv,ierror)
		call MPI_WAIT(request_recv,status,ierror)
		call MPI_ISEND(value_receive,1,MPI_INT,my_rank+1,tag,MPI_COMM_WORLD,request_send,ierror)
	        call MPI_WAIT(request_send,status,ierror)
	elseif (my_rank==nproc-1) then
		call MPI_IRECV(value_receive,1,MPI_INT,my_rank-1,MPI_ANY_TAG,MPI_COMM_WORLD,request_recv,ierror)
		call MPI_WAIT(request_recv,status,ierror)
		call MPI_ISEND(value_receive,1,MPI_INT,0,tag,MPI_COMM_WORLD,request_send,ierror)
		call MPI_WAIT(request_send,status,ierror)
	endif
