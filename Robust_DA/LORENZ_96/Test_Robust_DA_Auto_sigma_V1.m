...
PLC=0.3;
for sigma_i=1:4
    sigma_pos=[1e-4,1e-3,1e-2,1e-1];
    sigma=sigma_pos(sigma_i);
    
    R=sigma^2*eye(m);
 for exp=1:Nexp
     disp(['Sigma: ',num2str(sigma),' exp: ',num2str(exp) ])
    %=====Scenario EnTLHF===
    %==Observations===
    Xb=zeros(n,N,Tsim);
    Xb(:,:,1)=10*rand(n,N);
    ...
    error_EnTLHF_sigma(sigma_i,exp)=norm(abs(sum(meanxa_EnTLHF(:,:)-Xreal(:,:))))/sqrt(Tsim);
    ...
    error_EnTLHF_KA_sigma(sigma_i,exp)=norm(abs(sum(meanxa_EnTLHF_KA(:,:)-Xreal(:,:))))/sqrt(Tsim);
    ...
    error_EnKF_sigma(sigma_i,exp)=norm(abs(sum(meanxa_EnKF(:,:)-Xreal(:,:))))/sqrt(Tsim);
    ...
    error_EnKF_KA_sigma(sigma_i,exp)=norm(abs(sum(meanxa_EnKF_KA(:,:)-Xreal(:,:))))/sqrt(Tsim);
...
fig=figure;
error_EnTLHF_sigma(error_EnTLHF_sigma>1000)=NaN;
error_EnTLHF_KA_sigma(error_EnTLHF_KA_sigma>1000)=NaN;
error_EnKF_sigma(error_EnKF_sigma>1000)=NaN;
error_EnKF_KA_sigma(error_EnKF_KA_sigma>1000)=NaN;


rmse_EnTLHF=nanmedian(error_EnTLHF_sigma,2);
rmse_EnTLHF_KA=nanmedian(error_EnTLHF_KA_sigma,2);
rmse_EnKF=nanmedian(error_EnKF_sigma,2);
