...
n=100*110; %Number of states
ncolumn=110; %Number of columns
nrow=n/ncolumn;
% T=zeros(n);
T=RHO_advection;
Radius=10; %Radius of localization
r=Radius+2;
KA=0; %Include Knownledge
dist = (0 : ncolumn - 1);
coeffs = exp(-1* (dist / Radius) .^ 2);
%===Physical restrictions====
NRest=0; %Number of restrictions
Rest_vert=0;
Rest_hori=0;
for i=1:n
    i
    %===Detection of row and column
     ...
     for j=1:length(row_radius)
         for k=1:length(column_radius)
             ...
             if KA==1 && (abs(orog(row,column)-orog(row_radius(j),column_radius(k)) )>=200)
                T(i,state)=0;
                T(state,i)=0;
